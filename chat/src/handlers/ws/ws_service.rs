...
use crate::infra::repositories::groups::query_group_invitation_by_user_id;
...
async fn sync_offline_msg(
    ...
    // 查询离线消息
    match get_offline_msg(pg_pool, user_id.clone()).await {
        Ok(list) => {
            ...
            for msg in list {
                ...
                if let Err(err) = guard
                    ...
                    .await
                {
                    tracing::error!("send offline message errors: {:?}", err);
                }
            }
        }
        Err(err) => {
            tracing::error!("query offline message errors: {:?}", err);
        }
    }
        ...
        Ok(list) => {
            for msg in list {
                ...
                if let Err(err) = guard
                    ...
                    .await
                {
                    tracing::error!("send friendship request errors: {:?}", err);
                }
            }
        }
        Err(err) => {
            tracing::error!("query friendship request errors: {:?}", err);
        }
    }
        ...
        Ok(list) => {
            for msg in list {
                ...
                if let Err(err) = guard
                    ...
                    .await
                {
                    tracing::error!("send friendship response errors: {:?}", err);
                }
            }
        }
        Err(err) => {
            tracing::error!("query friendship response errors: {:?}", err);
        }
    }
    // query group invitation
    // because group invitation and members maybe huge, so we use tokio task to send it
            ...
            Ok(invitation) => {
                for msg in invitation {
                    ...
                    if let Err(err) = guard
                        ...
                        .await
                    {
                        tracing::error!("send group invitation errors: {:?}", err);
                    }
                }
            }
            Err(err) => {
                tracing::error!("query group invitation errors: {}", err);
            }
        };
    });
}
...
async fn websocket(user_id: String, pointer_id: String, ws: WebSocket, state: AppState) {
    ...
    if let Err(err) = update_online(&state.pg_pool, &user_id, true).await {
        tracing::error!("mark user online errors: {}", err);
    }
    // 注册完成后，查询离线消息，以同步的方式发送给客户端
    // FIXME to handle errors
    if let Ok(redis) = state.redis.get_connection() {
        sync_offline_msg(
            redis,
            &pool,
            &state.pg_pool,
