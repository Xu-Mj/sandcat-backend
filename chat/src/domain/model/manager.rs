        ...
        while let Some(message) = receiver.recv().await {
            match message.clone() {
                Msg::Single(msg) => {
                    ...
                    // 入库成功后给客户端回复消息已送达的通知
                    self.send_single_msg(&msg.receiver_id, &message).await;
                }
                Msg::Group(group_msg) => {
                    match group_msg {
                        GroupMsg::Message(msg) => {
                            ...
                            if let Ok(mut conn) = redis.get_connection() {
                                match query_group_members_id(&mut conn, &pg_pool, msg.receiver_id)
                                    .await
                                {
                                    Err(err) => {
                                        error!("查询群成员失败: {:?}", err);
                        ...
                        GroupMsg::InvitationReceived((user_id, group_id)) => {
                            ...
                            {
                                error!("update group invitation delivered with group id {group_id}; user id {user_id} errors: {:?}", err)
                            }
                        }
                    }
